            ...
            with tqdm(total=len(os.listdir(root)), desc="Processing ID") as pbar:
                for id_folder in filter(lambda d: d.isnumeric(), os.listdir(root)):
                    try:
                        ...
                        fod_info = read_dataset.get_sample_subject(f"{output_id_folder}/data.hdf5")
                        np.save(f"{output_id_folder}/index_wm.npy", fod_info["index_mask"])
                        os.remove(f"{output_id_folder}/data.hdf5")
                    except:
